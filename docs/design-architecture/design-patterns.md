# Arkhon-Rheo + Arkhon-Rheo System Design
...
sequenceDiagram
    ...
    Runtime->>Graph: run(initial_state)
    
    loop Until Terminated
        Graph->>ThoughtNode: run(state)
        ThoughtNode->>Tool: LLM.generate()
        Tool-->>ThoughtNode: thought
        ThoughtNode-->>Graph: updated_state
        
        Graph->>CheckpointMgr: save(state)
        
        Graph->>ActionNode: run(state)
        ActionNode->>Tool: execute(action)
        Tool-->>ActionNode: observation
        ActionNode-->>Graph: updated_state
    end
    
    Graph-->>Runtime: final_state
    Runtime-->>User: result
```
...
classDiagram
    class BaseNode {
        ...
        +before_run(state: AgentState) None
        +after_run(state: AgentState) None
    }
