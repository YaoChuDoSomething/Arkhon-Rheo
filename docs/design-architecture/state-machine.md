# Arkhon-Rheo + Arkhon-Rheo State Machine Design
...
While the primary state is the `current_node`, the FSM also maintains **extended state** in `AgentState.metadata`:
...
```python
def compute_next_node(state: AgentState) -> str:
...
class CheckpointManager:
    ...
    def load(self, trace_id: str) -> AgentState:
...
class RuntimeExecutor:
    def run(
        ...
        interrupt_fn: Callable[[AgentState], bool] | None = None
    ) -> AgentState:
        state = initial_state
...
```python
def replay_execution(trace_id: str, up_to_step: int) -> AgentState:
